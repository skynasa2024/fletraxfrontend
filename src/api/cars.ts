...
export const getVehicleDetails = async (id: string): Promise<VehicleDTO> => {
  const vehicle = await axios.get<ResponseModel<VehicleDTO>>(`/api/vehicles/cars/show/${id}`);
  return vehicle.data.result;
};
